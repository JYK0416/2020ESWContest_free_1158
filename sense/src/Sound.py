...
BlowerBound = np.array([128, 36, 144])
BupperBound = np.array([192, 93, 170])

x2 = 0
y2 = 0
area_1 = 0
def sound(hsv, sensor_position, x1, y1):
	...
	if len(Gcontours) > 0:
	        for i in range(len(Gcontours)):
	            ...
	            if area > 30:  # minimum yellow area
			area_1 = area
	...
	if x2 is not 0 and y2 is not 0:
		...
		if sensor_position is 2:
			if y2 < y1+5 and y2 > y1 -5:
				if x2 - x1 < 0:
					return 1, area_1 * 0.1 / abs(x2-x1) * 100
				else:
					return 0, 0
			...
			if x2 < x1+5 and x2 > x1 -5:
				if y2 - y1 < 0:
					return 1, area_1 * 0.1 / abs(y2-y1) * 100
				else:
					return 0,0
			else:
				return 0, 0
