...
BlowerBound = np.array([90, 92, 0])
BupperBound = np.array([111, 255, 255])

x2 = 0
y2 = 0
area = 0
def sonar(hsv, sensor_position, x1, y1):
	...
	if x2 is not 0 and y2 is not 0:
		...
		if sensor_position is 2:
			if y2 < y1+5 and y2 > y1 -5 and x2 - x1 < 0:
				return 1, abs(x2-x1)
			...
			if x2 < x1+5 and x2 > x1 -5 and y2 - y1 < 0:
				return 1, abs(y2 - y1)
